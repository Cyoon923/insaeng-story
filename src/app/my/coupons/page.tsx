  ...
  return (
    <MobileShell>
      ...
      <section className="px-4 py-5">
        <h2 className="font-serif text-[24px] font-bold text-[#3d2b1f]">쿠폰함</h2>
      ...
      <div className="space-y-3 px-4 pb-8">
        {loaded && !loggedIn ? (
          <div className="rounded-2xl bg-white px-5 py-12 text-center ring-1 ring-[#ebe3d8]">
            ...
            <p className="mt-4 text-[17px] font-bold text-[#3d2b1f]">로그인이 필요합니다</p>
            <p className="mt-2 text-[15px] leading-relaxed text-[#8b6f5c]">
              로그인하면 받은 쿠폰을 확인할 수 있습니다.
            </p>
            <Link
              href="/login"
              className="mt-6 inline-flex h-12 items-center justify-center rounded-full bg-[#5c3d2e] px-6 text-[15px] font-semibold text-white"
            >
              로그인하기
            </Link>
          </div>
        ...
        {loaded && loggedIn && coupons.length === 0 ? (
          <div className="rounded-2xl bg-white px-5 py-12 text-center ring-1 ring-[#ebe3d8]">
            <Ticket className="mx-auto h-10 w-10 text-[#8b6f5c]" strokeWidth={1.4} />
            <p className="mt-4 text-[17px] font-bold text-[#3d2b1f]">보유한 쿠폰이 없습니다</p>
            <p className="mt-2 text-[15px] leading-relaxed text-[#8b6f5c]">
              진행 중인 이벤트에서
              <br />
              혜택을 확인하실 수 있습니다.
        ...
        {coupons.map((coupon) => (
          <div key={coupon.id} className="rounded-2xl bg-white p-5 ring-1 ring-[#ebe3d8]">
            <div className="flex items-start gap-3">
              <Ticket className="mt-0.5 h-6 w-6 text-[#5c3d2e]" />
              <div>
