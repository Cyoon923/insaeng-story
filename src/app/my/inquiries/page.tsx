  ...
  return (
    <MobileShell>
      ...
      <section className="px-4 py-5">
        <h2 className="font-serif text-[24px] font-bold text-[#3d2b1f]">이벤트 신청</h2>
      ...
      <div className="space-y-3 px-4 pb-8">
        {loaded && !user ? (
          <div className="rounded-2xl bg-white p-5 text-center ring-1 ring-[#ebe3d8]">
            <p className="text-[15px] text-[#8b6f5c]">로그인하면 신청 내역을 볼 수 있습니다.</p>
            <Link
              href="/login"
        ...
        {items.map((item) => (
          <div key={item.id} className="rounded-2xl bg-white p-4 ring-1 ring-[#ebe3d8]">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h3 className="text-[17px] font-bold text-[#3d2b1f]">{inquiryTitle(item)}</h3>
                <p className="mt-1 text-[14px] text-[#8b6f5c]">신청일 {formatDate(item.createdAt)}</p>
              </div>
              <span className="shrink-0 rounded-full bg-[#e8f3ea] px-2.5 py-0.5 text-[12px] font-medium text-[#3d6b45]">
                신청접수
              </span>
            </div>
            <p className="mt-3 text-[15px] leading-relaxed text-[#5c3d2e]">{item.product}</p>
            {item.message ? (
