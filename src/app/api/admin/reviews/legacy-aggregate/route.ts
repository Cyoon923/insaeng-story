/**
 * 관리자 전용 과거 탈퇴 회원 후기 잔존 현황 확인 (Privacy-Reviews-Legacy-Diagnostic-2).
 *
 ...
 * 정해진 질의 하나를 보낼 수단을 규칙 모듈에 넘긴다. 판단도 질의문도
 * reviewLegacyAggregate.ts에 있다. 여기에는 SQL도, 조건도, 새 인증도 없다.
 *
 ...
 *
 * 응답에는 건수와 시각만 담긴다. 후기의 userId·이름·본문·대상(targetKey)과
 * 회원 정보·SQL·접속 정보·받은 오류는 담지 않는다.
 */
...
} from "@/lib/server/reviewLegacyAggregate";
  ...
  const response = await runReviewLegacyAggregate(
    {
      ...
      queryAggregate: async () => {
        ...
        return (await sql.query(REVIEW_LEGACY_AGGREGATE_SQL)) as readonly Record<
          string,
          unknown
        >[];
      },
