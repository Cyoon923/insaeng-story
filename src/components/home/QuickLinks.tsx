...
const LINKS = [
  ...
  { href: "/notice", icon: Megaphone, label: "공지사항", desc: "인생스토리의\n소식을 확인하세요" },
];

export function QuickLinks() {
  return (
    <section className="border-t border-[#ebe3d8] bg-[#faf8f5] px-4 py-4">
      <div className="flex divide-x divide-[#e0d5c8] rounded-2xl bg-white py-4 ring-1 ring-[#ebe3d8]">
        {LINKS.map(({ href, icon: Icon, label, desc }) => (
          <Link key={href} href={href} className="flex flex-1 flex-col items-center px-2 text-center">
            ...
            <span className="mt-1 whitespace-pre-line text-[9px] leading-snug text-[#8b6f5c]">{desc}</span>
          </Link>
        ))}
      </div>
    </section>
