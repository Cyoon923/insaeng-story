  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-center justify-center px-5">
      ...
      <div className="relative w-full max-w-[380px] rounded-2xl bg-white p-5 ring-1 ring-[#ebe3d8]">
        <p className="text-[20px] font-bold leading-snug text-[#403A49]">{note.title}</p>
        <p className="mt-3 text-[16px] leading-relaxed text-[#5c3d2e]">{note.body}</p>
        <button
          type="button"
          onClick={close}
          className="mt-5 flex h-12 w-full items-center justify-center rounded-xl bg-[#403A49] text-[16px] font-semibold text-white"
        >
          확인
        </button>
        {note.kind === "coupon" ? (
          <button
            type="button"
            onClick={goCoupons}
            className="mt-2 flex h-12 w-full items-center justify-center rounded-xl border border-[#403A49] bg-[#fffdf9] text-[16px] font-semibold text-[#403A49]"
          >
            쿠폰함 보기
          </button>
        ) : null}
