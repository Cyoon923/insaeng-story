...
/**
 * 관리자가 한 처리에 남기는 실행자 값.
 *
 * 지금 관리자 인증은 ADMIN_PASSWORD 하나뿐이라 "누가" 했는지 구분할 수 없다.
 * 세션 토큰에도 사람을 가리키는 값이 없다. 그래서 "이 단일 관리자 세션에서
 * 처리했다"는 뜻의 상수 하나를 쓴다.
 *
 * 클라이언트가 보낸 값을 실행자로 쓰지 않기 위해 서버 상수로 두고, 여러 파일에
 * "admin" 문자열을 흩뿌리지 않도록 관리자 세션을 다루는 이 파일 한곳에 둔다.
 * 나중에 관리자 계정이 생기면 이 값을 실제 식별자로 바꾸는 것이 출발점이다.
 */
export const CURRENT_ADMIN_ACTOR = "admin";
