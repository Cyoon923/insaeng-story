/**
 * 취소 성공 뒤 끊긴 건의 복구 테스트 (Refund-Payment-Succeeded-Recovery-1).
 *
 * 실행: node --test src/lib/server/refundPaymentSucceededRecovery.test.ts
 *
 * 실제 NICEPAY도 DB도 부르지 않는다. deps에는 취소·선점 함수가 아예 없으므로
 * 그 둘이 불리지 않는다는 사실은 호출 기록이 아니라 구조로 보장된다
 * (마지막 테스트에서 소스도 확인한다).
 */
import assert from "node:assert/strict";
import test from "node:test";
import { readFileSync } from "node:fs";
import { runRefundPaymentSucceededRecovery } from "./refundPaymentSucceededRecovery.ts";
import type { RefundPaymentSucceededRecoveryDeps } from "./refundPaymentSucceededRecovery.ts";
...
/** 취소는 성공으로 기록됐지만 아직 결제가 paid로 남아 있는 건. */
...
function harness(options: {
  ...
  reconcile?: RefundPaymentSucceededRecoveryDeps["reconcile"];
}) {
  const calls: string[] = [];
  ...
  const deps: RefundPaymentSucceededRecoveryDeps = {
...
test("정상: succeeded + PG 전액취소 → 조회 1회 → normal → 최종화 1회 → recovered-completed", async () => {
  const h = harness({});
  const result = await runRefundPaymentSucceededRecovery(ORDER_ID, h.deps);
...
const BLOCKED_BEFORE_INQUIRY: { name: string; lookup: OrderPaymentLookup; reason: string }[] = [
  { name: "processing", lookup: { kind: "single-paid-payment", payment: payment({ cancelExecutionStatus: "processing" }) }, reason: "not-succeeded-execution" },
  { name: "unknown", lookup: { kind: "single-paid-payment", payment: payment({ cancelExecutionStatus: "unknown" }) }, reason: "not-succeeded-execution" },
  { name: "declined", lookup: { kind: "single-paid-payment", payment: payment({ cancelExecutionStatus: "declined" }) }, reason: "not-succeeded-execution" },
  { name: "실행 단계 없음", lookup: { kind: "single-paid-payment", payment: payment({ cancelExecutionStatus: null }) }, reason: "not-succeeded-execution" },
  ...
  { name: "결제의 주문이 다름", lookup: { kind: "single-paid-payment", payment: payment({ orderId: "o-other" }) }, reason: "order-id-mismatch" },
  { name: "tid 없음", lookup: { kind: "single-paid-payment", payment: payment({ pgTid: "" }) }, reason: "missing-tid" },
  { name: "승인 금액 이상", lookup: { kind: "single-paid-payment", payment: payment({ approvedAmount: 0 }) }, reason: "invalid-approved-amount" },
  { name: "이미 취소 금액 있음", lookup: { kind: "single-paid-payment", payment: payment({ cancelledAmount: 50000 }) }, reason: "already-cancelled" },
];
...
const BLOCKED_INQUIRIES: { name: string; inquiry: NicepayPaymentInquiryOutcome; kind: string }[] = [
  ...
  {
    name: "금액 근거 없음",
    inquiry: inquiryFound({ cancelledAmt: undefined, balanceAmt: undefined }),
    kind: "not-recoverable",
  },
];
...
for (const source of ["recovered-unknown", "recovered-stale-claim"] as const) {
  test(`확정 경로 ${source} → 최종화 0`, async () => {
    const h = harness({ reconcile: () => VERIFIED(source) });
    const result = await runRefundPaymentSucceededRecovery(ORDER_ID, h.deps);
    assert.equal(result.kind, "not-recoverable");
    if (result.kind !== "not-recoverable") return;
...
test("확정 경로 normal → 최종화 1회", async () => {
 ...
 * 같으며 완료된 환불 문의가 있다"를 확인한 뒤에만 나온다. DB가 이미 완료 상태라는
 * 뜻이 맞으므로 복구 완료로 읽되 이번 호출이 아무것도 바꾸지 않았음을 표시한다.
 */
...
test("최종화 도중 예외 → recovery-failed, 상태를 되돌리지 않는다", async () => {
  ...
  const result = await runRefundPaymentSucceededRecovery(ORDER_ID, deps);
  assert.equal(result.kind, "recovery-failed");
  assert.deepEqual(h.calls, ["lookup", "inquiry", "finalize"]);
});

/*
 * 두 번째 실행. 첫 실행으로 결제가 cancelled가 되면 findPaidPaymentForOrder가
 * 더 이상 찾지 못해 no-payment로 끝난다. PG를 다시 부르지 않는다.
 */
test("복구가 끝난 뒤 다시 실행하면 조회 없이 끝난다", async () => {
...
test("이 모듈은 취소·선점·정상 완결 흐름을 참조하지 않는다", () => {
  const source = readFileSync(
    new URL("./refundPaymentSucceededRecovery.ts", import.meta.url),
    "utf8",
  );
...
test("복원에는 주문 id만 넘긴다. 금액도 회원도 넘기지 않는다", async () => {
  const h = harness({});
  await runRefundPaymentSucceededRecovery(ORDER_ID, h.deps);
...
const RESTORE_RESULTS: { name: string; restore: RefundPointsRestoreResult }[] = [
  ...
  {
    name: "not-applicable",
    restore: { kind: "not-applicable", orderId: ORDER_ID, reason: "no-points-used" },
  },
  {
    name: "retry-later",
...
test("확정 경로가 normal이 아니면 복원 0회", async () => {
  for (const source of ["recovered-unknown", "recovered-stale-claim"] as const) {
    const h = harness({ reconcile: () => VERIFIED(source) });
    await runRefundPaymentSucceededRecovery(ORDER_ID, h.deps);
    assert.equal(h.restoreInputs.length, 0, source);
  }
});

test("최종화 실패·예외 경로에서는 복원 0회", async () => {
  const failed = harness({ finalize: { ok: false, kind: "payment-mismatch" } });
  await runRefundPaymentSucceededRecovery(ORDER_ID, failed.deps);
