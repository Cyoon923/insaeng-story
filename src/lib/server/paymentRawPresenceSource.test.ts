/**
 * PG 응답 원문 현황 경로의 경계 테스트 (Privacy-Payment-Raw-Presence-1).
 *
 * 실행: node --test src/lib/server/paymentRawPresenceSource.test.ts
 *
 * 규칙 자체는 paymentRawPresence.test.ts가 본다. 여기서는 이 경로가
 * **저장소를 바꾸지 않는다**는 것, 관리자 경계, 그리고 이 경로가 다른 요청에
 * 끼어들지 않는다는 것만 본다.
 *
...
const ROUTE_PATH = "../../app/api/admin/payments/raw-presence/route.ts";
const ROUTE = readFileSync(new URL(ROUTE_PATH, import.meta.url), "utf8");
const MODULE = readFileSync(new URL("./paymentRawPresence.ts", import.meta.url), "utf8");
...
test("관리자 확인이 DB 접근보다 먼저다", () => {
  ...
  const query = body.indexOf("sql.query(");
  const run = body.indexOf("runPaymentRawPresence(");
  assert.notEqual(guard, -1);
  assert.ok(guard < now, "관리자 확인보다 기준 시각을 먼저 만든다");
  assert.ok(guard < client, "관리자 확인보다 저장소에 먼저 닿는다");
  assert.ok(guard < query, "관리자 확인보다 질의를 먼저 보낸다");
...
test("응답 타입에 배열이나 자유 키가 없다", () => {
  const bodyType = MODULE_CODE.slice(
    MODULE_CODE.indexOf("interface RawColumnPresence"),
    MODULE_CODE.indexOf("export const PAYMENT_RAW_PRESENCE_ERRORS"),
  );
...
test("받은 오류를 응답에 담지 않는다", () => {
  assert.equal([...MODULE_CODE.matchAll(/\}\s*catch\s*\{/g)].length, 2);
...
test("이 모듈을 쓰는 Production 파일은 전용 route 하나뿐이다", () => {
  ...
  const walk = (dir: URL) => {
    for (const entry of readdirSync(dir)) {
      ...
      if (text.includes("paymentRawPresence")) hits.push(child.pathname);
    }
  };
  for (const root of roots) walk(root);
  const importers = hits.filter((path) => !path.endsWith("paymentRawPresence.ts"));
  assert.deepEqual(
    importers.map((path) => path.slice(path.indexOf("/src/"))),
    ["/src/app/api/admin/payments/raw-presence/route.ts"],
  );
});

test("복구 현황 경로와 서로 의존하지 않는다", () => {
  for (const [where, source] of BOTH) {
    assert.equal(
      /paymentRecoveryAggregate/.test(source),
      false,
      `${where}가 복구 현황 모듈에 의존한다`,
    );
  }
  // 반대 방향도 본다. 기존 경로가 이 파일을 알게 되면 경계가 섞인다.
  const recovery = readFileSync(new URL("./paymentRecoveryAggregate.ts", import.meta.url), "utf8");
  const recoveryRoute = readFileSync(
    new URL("../../app/api/admin/payments/recovery-aggregate/route.ts", import.meta.url),
    "utf8",
  );
  assert.equal(/paymentRawPresence/.test(recovery), false);
  assert.equal(/paymentRawPresence/.test(recoveryRoute), false);
});
